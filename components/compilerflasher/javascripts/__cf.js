compilerflasher = function(lf){
    ...
    this.pluginHandler = new function(){
        ...
        this.clickedPort = function()
        {
            ...
            url = url.replace("CLICK_PORT_META", JSON.stringify({ "selectedPort": port} ));
            $.get(url);
        }
        ...
        this.savePort = function(port) {
            ...
            if(typeof Lawnchair !== "undefined")
            {
                ...
                new Lawnchair(function () {
                    ...
                    url = url.replace("SAVEPORTMETA", JSON.stringify({ "oldPort":oldPort, "newPort": newPort } ));
                    $.get(url);
        ...
        this.initializePlugin = function()
        {
            ...
            if (location.indexOf("codebender.cc") != -1 && !window.osBrowserIsSupported() )
            {
                ...
                {
                    this.owner.setOperationOutput('<i class="icon-warning-sign"></i> Sorry! We only support <a href="http://www.google.com/chrome/" target="_blank">Google Chrome</a> and <a href="http://www.mozilla.org/en-US/firefox/" target="_blank">Mozilla Firefox</a>.');
                    this.owner.eventManager.fire('plugin_notification', '<i class="icon-warning-sign"></i> Sorry! We only support <a href="http://www.google.com/chrome/" target="_blank">Google Chrome</a> and <a href="http://www.mozilla.org/en-US/firefox/" target="_blank">Mozilla Firefox</a>.')
                }
                ...
                {
                    this.owner.setOperationOutput('<i class="icon-warning-sign"></i> Sorry! We only support <a href="http://www.google.com/chrome/" target="_blank">Google Chrome (up to version 34 on Linux)</a> and <a href="http://www.mozilla.org/en-US/firefox/" target="_blank">Mozilla Firefox</a>.');
                    this.owner.eventManager.fire('plugin_notification', '<i class="icon-warning-sign"></i> Sorry! We only support <a href="http://www.google.com/chrome/" target="_blank">Google Chrome (up to version 34 on Linux)</a> and <a href="http://www.mozilla.org/en-US/firefox/" target="_blank">Mozilla Firefox</a>.');
                }
                else
                {
                    this.owner.setOperationOutput('<i class="icon-warning-sign"></i> Sorry! We only support <a href="http://www.google.com/chrome/" target="_blank">Google Chrome</a> and <a href="http://www.mozilla.org/en-US/firefox/" target="_blank">Mozilla Firefox</a> from Windows, Mac or Linux.');
                    this.owner.eventManager.fire('plugin_notification','<i class="icon-warning-sign"></i> Sorry! We only support <a href="http://www.google.com/chrome/" target="_blank">Google Chrome</a> and <a href="http://www.mozilla.org/en-US/firefox/" target="_blank">Mozilla Firefox</a> from Windows, Mac or Linux.');
                }
            }
            else
            {
        ...
        this.initializePluginPortsLogger = function()
        {
            ...
            window.PluginLoggingInterval = setInterval(function(){
                try
                {
                    ...
                    {
                        if ((oldPortsAvail.length < portsAvail.length || (oldPortsAvail.length == 1 && portsAvail.length == 1 && oldPortsAvail[0] == "" && portsAvail[0] != ""))
                        ...
                        if (typeof(document.getElementById('plugin0').getPorts) !== "undefined")
                        {
                            ...
                            {
                                var parsedList = JSON.parse(serialPortsAvail);
                                var url = "https\x3A\x2F\x2Fcodebender.cc\x2Futilities\x2Flogdb\x2F74\x2FPLUGIN_META";
                                url = url.replace("PLUGIN_META", JSON.stringify({ "success": true, "plugin" : true, "version": document.getElementById('plugin0').version, "probeUSBports" : document.getElementById('plugin0').probeUSB(), "serialLibPorts" : oldPortsAvail}) );
                                $.get(url);
	                            var url = "https\x3A\x2F\x2Fcodebender.cc\x2Futilities\x2Flogdb\x2F75\x2FPLUGIN_META";
	                            url = url.replace("PLUGIN_META", JSON.stringify({ "success": true, "plugin" : true, "version": document.getElementById('plugin0').version, "jsonPorts" : parsedList}) );
	                            $.get(url);
                            }
...
//        alertElement += "<button type='button' class='close' data-dismiss='alert'>x</button>";
        ...
        this.connect = function() {
            ...
            if (this.connected == false) {
                ...
                url = url.replace("SERIAL_MONITOR_META", JSON.stringify({ "baudrate" : speed, "port": $("#cb_cf_ports").val()}) );
                $.get(url);
        ...
        this.disconnect = function(notified) {
            ...
            if (this.connected == true) {
                if(notified == false)
                {
                    ...
                    url = url.replace("SERIAL_MONITOR_DISC_META", JSON.stringify({ "baudrate" : $("#cb_cf_baud_rates option:selected").val(), "port": $("#cb_cf_ports").val()}) );
                    $.get(url);
                }
    ...
    // ----------------------------------------------------------------------------- ATTTTTTTAAAACCHHHH EVENT FIRE ON BUTTON
    ...
    this.saveBoard = function() {
        ...
        url = url.replace("SAVEBOARDMETA", JSON.stringify({ "oldBoard":oldBoard, "newBoard":newBoard } ));
        $.get(url);
    };
    ...
    this.saveProgrammer = function() {
        ...
        url = url.replace("SAVEPROGRAMMERMETA", JSON.stringify({ "oldProgrammer":oldProgrammer, "newProgrammer":newProgrammer } ));
        $.get(url);
    ...
    this.clickedBoard = function()
    {
        ...
        url = url.replace("CLICK_BOARD_META", JSON.stringify({ "selectedBoard": board} ));
        $.get(url);
    }
    ...
    this.clickedProgrammer = function()
    {
        ...
        url = url.replace("CLICK_PROGRAMMER_META", JSON.stringify({ "selectedProgrammer": programmer} ));
        $.get(url);
    }
    ...
    this.getHex = function() {
        ...
        $.post("https\x3A\x2F\x2Fcodebender.cc\x2Futilities\x2Fcompile\x2F", payload, function (data) {
            try{
                ...
                if (obj.success == 0) {
                    cb.setOperationOutput("verification failed_1.");
    ...
    this.usbflash = function()
    {
        ...
        url = url.replace("RUN_BUTTON_META", JSON.stringify({ "port":$("#cb_cf_ports option:selected").text(), "board":$("#cb_cf_boards option:selected").text(), "programmer":$("#cb_cf_programmers option:selected").text() } ));
        $.get(url);
        ...
        if(this.pluginHandler.canflash(this.selectedBoard, this.selectedProgrammer))
        {
            ...
            this.setOperationOutput("<i class='icon-spinner icon-spin'> </i> woking_1...");
    ...
    this.usbflashWithProgrammer = function()
    {
        ...
        url = url.replace("RUN_WITH_PROG_BUTTON_META", JSON.stringify({ "port":$("#cb_cf_ports option:selected").text(), "board":$("#cb_cf_boards option:selected").text(), "programmer":$("#cb_cf_programmers option:selected").text() } ));
        $.get(url);
        ...
        if(this.pluginHandler.canflash(this.selectedBoard, this.selectedProgrammer, true))
        {
            var cb = this;
            this.eventManager.fire('pre_flash');
            this.setOperationOutput("<i class='icon-spinner icon-spin'> </i> woking_2...");
            this.getbin(function(obj){
                if (obj.success == 0) {
                    cb.setOperationOutput("There was an error compiling.")
                    cb.eventManager.fire('verification_failed', obj.message);
...
//----------------------------------------------------------------- FFFFFFFFFFFFFFFFFFFFFFFFFFFIIIIRRRRRRRREEEEE
    ...
    this.verify =  function() {
        ...
        url = url.replace("VERIFY_META", JSON.stringify({ "selectedBoard": board} ));
        $.get(url);

        this.eventManager.fire('pre_verify');
        this.setOperationOutput("<i class='icon-spinner icon-spin'> </i> woking_3...");
        var cb = this;
        this.getbin(function(obj){
    
            if (obj.success == 0) {
x('fail....');        
                cb.setOperationOutput("verification failed_2.");
                cb.eventManager.fire('verification_failed', obj.message);
            }
            else
            {
x('success !!!!!.');        
    ...
    this.burn_bootloader = function() {
        ...
        if(this.pluginHandler.canBurnBootloader(this.selectedProgrammer))
        {
            ...
            this.setOperationOutput("<i class='icon-spinner icon-spin'></i> woking_4...")
